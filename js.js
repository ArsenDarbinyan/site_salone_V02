...
async function toggleServiceCard(serviceId) {
    ...
    if (isExpanding) {
        // 1. При раскрытии сразу перемещаем вниз
        card.style.order = '9999';
        
        // Запускаем саму анимацию раскрытия
        const togglePromise = runExpandableCardToggle(serviceToggleGenRef, '.service-card', card, {
        ...
        // 2. Параллельно с началом анимации плавно скроллим вниз
        const grid = document.getElementById('servicesGrid');
        if (grid) {
            // Рассчитываем позицию с запасом, чтобы карточка была видна полностью
            const gridRect = grid.getBoundingClientRect();
            const gridBottom = gridRect.top + window.pageYOffset + gridRect.height;
            window.scrollTo({
                top: gridBottom - window.innerHeight + 260, // Смещаем чуть ниже (260px), чтобы скрыть верхние карточки
                behavior: 'smooth'
            });
        }
        ...
        // 1. При закрытии сначала запускаем анимацию сворачивания (находясь внизу списка)
        ...
        // 2. Только после завершения анимации закрытия возвращаем исходный order карточке
        const originalOrder = parseInt(card.getAttribute('data-original-order') || '0', 10);
        card.style.order = originalOrder;
        
        // 3. Плавно скроллим экран точно к этой карточке на её исходном месте
        setTimeout(() => {
            const offset = -100;
        ...
        }, 50); // Минимальная задержка, чтобы браузер успел применить order и рассчитать новые координаты
    }
}
